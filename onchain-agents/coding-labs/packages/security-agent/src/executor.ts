 ...
 * Unlike midnight-agent, this executor does not track session context since
 * security analysis skills are stateless.
 */
export class SecurityAgentExecutor implements AgentExecutor {
  ...
  public cancelTask = async (
    ...
    console.log(`[SecurityAgent] Cancellation requested for task: ${taskId}`);
  ...
  async execute(
    ...
    console.log(
      `[SecurityAgent] Processing message ${userMessage.messageId} for task ${taskId}`
    );
    ...
    console.log(`[SecurityAgent] Detected skill: ${skillId}`);
    ...
    try {
      ...
      for await (const event of handler(userMessage)) {
        ...
        if (this.cancelledTasks.has(taskId)) {
          console.log(`[SecurityAgent] Task ${taskId} cancelled`);
      ...
      console.log(`[SecurityAgent] Task ${taskId} completed successfully`);
    } catch (error) {
      const errorMessage =
        error instanceof Error ? error.message : 'Unknown error';
      console.error(`[SecurityAgent] Task ${taskId} failed:`, error);
