...
/**
 * Sonic Agent Executor
 *
 * Handles incoming A2A messages and routes them to appropriate skill handlers.
 */
export class SonicAgentExecutor implements AgentExecutor {
  ...
  public cancelTask = async (
    ...
    console.log(`[SonicAgent] Cancellation requested for task: ${taskId}`);
  ...
  async execute(
    ...
    console.log(
      `[SonicAgent] Processing message ${userMessage.messageId} for task ${taskId}`
    );
    ...
    console.log(
      `[SonicAgent] Detected skill: ${skillId}, wallet: ${walletContext?.address || 'not connected'}`
    );
    ...
    try {
      ...
      // Pass contextId as sessionId for MCP operations
      for await (const event of handler(
        ...
        if (this.cancelledTasks.has(taskId)) {
          console.log(`[SonicAgent] Task ${taskId} cancelled`);
      ...
      console.log(`[SonicAgent] Task ${taskId} completed successfully`);
    } catch (error) {
      const errorMessage =
        error instanceof Error ? error.message : 'Unknown error';
      console.error(`[SonicAgent] Task ${taskId} failed:`, error);
